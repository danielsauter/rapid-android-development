...
###Map the United States
 ...
 We’ll implement this project in two steps. First we load the map using `loadShape()`, and then we’ll draw it on the Android screen with Processing’s `shape()` method. Second, we’ll alter the fill color of states that political poll takers typically consider as swing states during recent presidential elections and draw them as purple shapes on the screen, as shown in Figure 12.0. In both cases, to zoom and pan the map we’ll return to the multitouch gestures we learned in <!--ref linkend="chp.display-->. 
