...
###Generate a Low-Resolution Preview
 ...
 In the main tab we also <keyword>import</keyword> the Ketai camera package, which is familiar to us from <!--ref linkend="chp.camera-->. We’ll create a `KetaiCamera` object that we’ll name `cam`. The `cam` object will read the image each time we receive a new frame from the camera. 
 ...
 8. 
 Call `interlace()` recursively from within itself using a new location and half the width and height of the previous call as parameters. 
 9. 
