...
For this project, we'll use a web server to facilitate sharing, and we'll equip it with a simple PHP script that takes the location info from each device and writes it to a text file. If one device knows the (made-up) name of the other, it can look it up on that server and we'll have a significant-other location to navigate to. You can certainly [download the script]() <!-- 4.6 NOTE - Update link when Appendix is addead -->, and host it on your own web server as well.
...
<!-- 4.7 NOTE - requires INTERNET permission and fullScreen(). Runs, but with other errors - after tap, distance displays as 0, & other errors noted in console -->
